...
// Note: Advanced AI modules are used internally by UCW for Claude Code enhancement
// but not exposed as standalone CLI commands to maintain Claude Code workflow focus

program
  .name('ucw')
  .description('Universal Claude Workflow - Supercharge your Claude Code sessions with intelligent project setup and enhanced context')
...
// Enhanced Claude Code preparation
program
  .command('prepare')
  .description('Prepare enhanced project context for Claude Code session')
  .option('-f, --focus <focus>', 'Development focus (tdd, security, performance, etc.)')
  .option('--update-context', 'Regenerate project context and CLAUDE.md')
  .action(async (options) => {
    try {
      console.log(chalk.blue.bold('🧭 Preparing Enhanced Claude Code Session\n'));
      ...
      if (options.updateContext) {
        console.log(chalk.blue('\nRegenerating project context for Claude Code...'));
        await installer.updateProjectContext(projectInfo, options.focus);
        console.log(chalk.green('✅ Context updated'));
      }
      
      console.log(chalk.green.bold('\n🎉 Claude Code session preparation complete!'));
      console.log(chalk.cyan('\nNext steps:'));
      console.log(chalk.cyan('  1. Review the generated CLAUDE.md file'));
      console.log(chalk.cyan('  2. Run "claude" to start your enhanced session'));
      console.log(chalk.cyan('  3. Try slash commands like "/tdd-cycle" or "/project-health"'));
